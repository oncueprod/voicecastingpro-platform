...
// RESTORED: Serve static files in production
...
// API-only root endpoint for development/debugging
app.get('/api', (req, res) => {
  res.json({
...
// RESTORED: Handle SPA routing in production - MUST come after all API routes
if (process.env.NODE_ENV === 'production') {
  app.get('*', (req, res) => {
    console.log('🔄 SPA route request:', req.path);
    
    // Only serve frontend for non-API requests
...
server.listen(PORT, () => {
  ...
  console.log('🌐 FRONTEND: Static file serving enabled for production');
