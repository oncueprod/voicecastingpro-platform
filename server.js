...
// Email configuration with voicecastingpro.com
let transporter;
...
async function initializeDatabase() {
  try {
    ...
    // Check if projects table needs to be updated with correct structure
    try {
      // First, check if the estimated_duration column exists and its type
      ...
      if (tableInfo.rows.length === 0) {
        console.log('🔧 estimated_duration column missing, adding it...');
        await client.query(`
          ALTER TABLE projects 
          ADD COLUMN estimated_duration VARCHAR(100)
        `);
      } else if (tableInfo.rows[0].data_type === 'integer') {
        console.log('🔧 estimated_duration has wrong type (integer), fixing...');
        // Drop and recreate the column with correct type
        await client.query(`ALTER TABLE projects DROP COLUMN estimated_duration`);
        await client.query(`ALTER TABLE projects ADD COLUMN estimated_duration VARCHAR(100)`);
      }
      
      console.log('✅ Projects estimated_duration column fixed');
    } catch (error) {
      console.log('❌ Error fixing estimated_duration:', error.message);
    }
    
    // Add other missing columns to projects table
    const projectColumns = [
      'ALTER TABLE projects ADD COLUMN IF NOT EXISTS client_email VARCHAR(255)',
      'ALTER TABLE projects ADD COLUMN IF NOT EXISTS status VARCHAR(20) DEFAULT \'open\'',
      'ALTER TABLE projects ADD COLUMN IF NOT EXISTS budget_min INTEGER',
      'ALTER TABLE projects ADD COLUMN IF NOT EXISTS budget_max INTEGER'
    ];
    
    for (const columnQuery of projectColumns) {
      try {
        await client.query(columnQuery);
      } catch (error) {
        console.log('ℹ️ Project column already exists');
      }
    }
    console.log('✅ Projects table schema updated');
    
    // Add missing columns to talent_profiles table
    const talentProfileColumns = [
      'ALTER TABLE talent_profiles ADD COLUMN IF NOT EXISTS rating DECIMAL(3,2) DEFAULT 5.0',
      'ALTER TABLE talent_profiles ADD COLUMN IF NOT EXISTS completed_projects INTEGER DEFAULT 0',
      'ALTER TABLE talent_profiles ADD COLUMN IF NOT EXISTS demo_count INTEGER DEFAULT 0',
      'ALTER TABLE talent_profiles ADD COLUMN IF NOT EXISTS subscription_active BOOLEAN DEFAULT FALSE',
      'ALTER TABLE talent_profiles ADD COLUMN IF NOT EXISTS subscription_type VARCHAR(20)',
      'ALTER TABLE talent_profiles ADD COLUMN IF NOT EXISTS subscription_expires_at TIMESTAMP'
    ];
    
    for (const columnQuery of talentProfileColumns) {
      try {
        await client.query(columnQuery);
      } catch (error) {
        console.log('ℹ️ Talent profile column already exists');
      }
    }
    console.log('✅ Talent profiles table schema updated');
...
// ✅ NEW: Password reset (complete the reset with new password)
...
// Apply to project - NEW ENHANCED VERSION
app.post('/api/projects/:projectId/apply', authenticateToken, async (req, res) => {
  try {
    ...
    // Get project details and client information
    ...
    const talentResult = await pool.query(`
      SELECT u.first_name, u.last_name, u.email, tp.bio, tp.languages, 
             tp.specialties, tp.hourly_rate_min, tp.hourly_rate_max, tp.rating
      FROM users u
    ...
    const clientEmail = project.client_email || `${project.client_first_name.toLowerCase()}@example.com`;
    
    const mailOptions = {
      to: clientEmail,
      subject: `New Application for Your Project: ${project.title}`,
      html: `
        <h2>🎤 New Voice Talent Application</h2>
        <p>Hi ${project.client_first_name},</p>
        
        <p>Great news! A voice talent has applied to your project on VoicecastingPro:</p>
        
        <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin: 20px 0; border-left: 4px solid #667eea;">
          <h3 style="color: #667eea; margin-bottom: 10px;">📋 Your Project</h3>
          <p><strong>Title:</strong> ${project.title}</p>
          <p><strong>Budget:</strong> $${project.budget_min}-${project.budget_max}</p>
        </div>
        
        <div style="background: #f1f8ff; padding: 20px; border-radius: 10px; margin: 20px 0; border-left: 4px solid #00d4ff;">
          <h3 style="color: #0066cc; margin-bottom: 15px;">🎭 Talent Profile</h3>
          <p><strong>Name:</strong> ${talent.first_name} ${talent.last_name}</p>
          <p><strong>Rating:</strong> ${'★'.repeat(Math.floor(talent.rating || 5))} ${talent.rating || 5}.0/5</p>
          <p><strong>Rate:</strong> $${talent.hourly_rate_min}-${talent.hourly_rate_max}/hour</p>
          <p><strong>Languages:</strong> ${talent.languages || 'Not specified'}</p>
          <p><strong>Specialties:</strong> ${talent.specialties || 'Not specified'}</p>
          ${proposedRate ? `<p><strong>Proposed Rate for Your Project:</strong> $${proposedRate}</p>` : ''}
          ${timeline ? `<p><strong>Proposed Timeline:</strong> ${timeline}</p>` : ''}
        </div>
        
        ${coverLetter ? `
        <div style="background: #fff8f0; padding: 20px; border-radius: 10px; margin: 20px 0; border-left: 4px solid #ff9500;">
          <h3 style="color: #cc7a00; margin-bottom: 15px;">💬 Cover Letter</h3>
          <p style="font-style: italic;">"${coverLetter}"</p>
        </div>
        ` : ''}
        
        <div style="background: #f0fff4; padding: 20px; border-radius: 10px; margin: 20px 0; border-left: 4px solid #28a745;">
          <h3 style="color: #155724; margin-bottom: 15px;">📞 Next Steps</h3>
          <p>To discuss your project with ${talent.first_name}:</p>
          <p><strong>Email:</strong> <a href="mailto:${talent.email}?subject=Re: ${project.title} Application">${talent.email}</a></p>
          <p>You can also <a href="${process.env.FRONTEND_URL || 'https://voicecastingpro.onrender.com'}/browse-talent" style="color: #667eea;">browse more talent profiles</a> on VoicecastingPro.</p>
        </div>
        
        <hr style="margin: 30px 0; border: none; border-top: 1px solid #eee;">
        <p style="color: #6c757d; font-size: 14px;">
          This application was sent through <a href="${process.env.FRONTEND_URL || 'https://voicecastingpro.onrender.com'}" style="color: #667eea;">VoicecastingPro</a> - 
          The premier platform for professional voice casting.
        </p>
      `,
    ...
    // Also send confirmation to talent
    const talentConfirmationOptions = {
      to: talent.email,
      subject: `Application Submitted: ${project.title}`,
      html: `
        <h2>✅ Application Submitted Successfully</h2>
        <p>Hi ${talent.first_name},</p>
        
        <p>Your application has been sent to the client for the project:</p>
        
        <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin: 20px 0;">
          <h3 style="color: #667eea;">${project.title}</h3>
          <p><strong>Client:</strong> ${project.client_first_name} ${project.client_last_name}</p>
          <p><strong>Budget:</strong> $${project.budget_min}-${project.budget_max}</p>
          <p><strong>Timeline:</strong> ${project.estimated_duration || 'Not specified'}</p>
        </div>
        
        <p>The client will review your application and contact you directly if interested. You can expect to hear back within 24-48 hours.</p>
        
        <p>Good luck!</p>
        <p>The VoicecastingPro Team</p>
      `
    };
    
    await sendEmail(talentConfirmationOptions);
    
    res.json({ 
      success: true, 
      message: 'Application sent to client successfully! They will contact you directly if interested.' 
    });
