// Serve audio demo files (updated for current database schema)
app.get('/api/demos/:id', async (req, res) => {
    res.status(501).json({ 
        error: 'Demo file serving not available with current database schema',
        message: 'Audio demos are stored as file references only'
    });
...
async function initDatabase() {
    ...
    try {
        ...
        // Audio demos table (compatible with existing schema)
        await client.query(`
        ...
        // Add new columns if they don't exist (for future use)
        try {
            await client.query(`ALTER TABLE audio_demos ADD COLUMN IF NOT EXISTS file_data BYTEA`);
            await client.query(`ALTER TABLE audio_demos ADD COLUMN IF NOT EXISTS file_name VARCHAR(255)`);
            await client.query(`ALTER TABLE audio_demos ADD COLUMN IF NOT EXISTS mime_type VARCHAR(100)`);
        } catch (alterError) {
            console.log('Note: Could not add new columns to audio_demos (this is normal)');
        }
...
app.get('/api/debug/user', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (user.account_type === 'talent') {
            ...
            talentProfile = profileResult.rows[0] || null;
        }
        
        res.json({
            user: user,
            talentProfile: talentProfile,
            isSubscribed: talentProfile ? talentProfile.subscription_active : false
        });
        
    } catch (error) {
        handleError(res, error, 'Failed to get user debug info');
    } finally {
        client.release();
    }
});
...
app.post('/api/talent/demos', authenticateToken, upload.array('audioDemo', 5), async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < req.files.length; i++) {
            ...
            // Store file name in file_path column to match existing schema
            const result = await client.query(
                'INSERT INTO audio_demos (talent_id, file_path, description, file_size) VALUES ($1, $2, $3, $4) RETURNING id, file_path, description, file_size, created_at',
                [talentId, file.originalname, description, file.size]
            );
    ...
    try {
        ...
        for (let talent of result.rows) {
            const demosResult = await client.query(
                'SELECT id, file_path as file_name, description, file_size, created_at FROM audio_demos WHERE talent_id = $1 LIMIT 3', 
                [talent.id]
            );
        ...
        console.log('Talent profiles:', result.rows.map(t => `${t.first_name} ${t.last_name} (subscription: ${t.subscription_active})`));
