...
async function initializeDatabase() {
  try {
    ...
    // Add/update missing columns in projects table
    const projectColumns = [
      'ALTER TABLE projects ADD COLUMN IF NOT EXISTS client_email VARCHAR(255)',
      'ALTER TABLE projects ADD COLUMN IF NOT EXISTS estimated_duration VARCHAR(100)',
    ...
    for (const columnQuery of projectColumns) {
      try {
        ...
        console.log('ℹ️ Project column already exists:', error.message);
      }
    }
    ...
    for (const columnQuery of talentProfileColumns) {
      try {
        ...
        console.log('ℹ️ Talent profile column already exists:', error.message);
      }
    }
