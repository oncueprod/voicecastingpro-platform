...
// Environment validation function (but don't call it immediately)
    ...
    try {
        ...
        if (userResult.rows.length > 0) {
            ...
            await client.query(
                ...
                [resetToken, new Date(Date.now() + 3600000), user.id] // 1 hour from now
            );
            ...
            if (transporter) {
                try {
                    ...
                    await transporter.sendMail({
                        from: process.env.EMAIL_USER,
                        to: email,
                        subject: 'Reset Your VoicecastingPro Password',
                        html: `
                            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                                <div style="text-align: center; margin-bottom: 30px;">
                                    <h1 style="color: #667eea; margin-bottom: 10px;">VoicecastingPro</h1>
                                    <h2 style="color: #333; margin-bottom: 20px;">Password Reset Request</h2>
                                </div>
                                
                                <div style="background: #f8f9fa; border-radius: 10px; padding: 25px; margin-bottom: 25px;">
                                    <p style="margin-bottom: 15px;">Hello ${user.first_name},</p>
                                    <p style="margin-bottom: 15px;">We received a request to reset your password for your VoicecastingPro account.</p>
                                ...
                                <div style="text-align: center; font-size: 12px; color: #666; border-top: 1px solid #eee; padding-top: 20px;">
                                    <p>© 2025 VoicecastingPro. All rights reserved.</p>
                                    <p>If you need help, contact us at support@voicecastingpro.com</p>
                                </div>
                            </div>
                        `
                    });
        ...
        handleError(res, error, 'Password reset request failed');
    } finally {
        client.release();
    }
});
...
app.post('/api/auth/password-reset-confirm', async (req, res) => {
    ...
    try {
        ...
        // Check if token matches and hasn't expired (optional extra security)
        ...
        if (transporter) {
            try {
                await transporter.sendMail({
                    from: process.env.EMAIL_USER,
                    to: user.email,
                    subject: 'Password Successfully Reset - VoicecastingPro',
                    html: `
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                            <div style="text-align: center; margin-bottom: 30px;">
                                <h1 style="color: #667eea; margin-bottom: 10px;">VoicecastingPro</h1>
                                <h2 style="color: #4ecdc4; margin-bottom: 20px;">✅ Password Reset Successful</h2>
                            </div>
                            
                            <div style="background: #d4edda; border: 1px solid #c3e6cb; border-radius: 10px; padding: 25px; margin-bottom: 25px;">
                                <p style="margin-bottom: 15px; color: #155724;">Your VoicecastingPro password has been successfully reset.</p>
                                <p style="margin-bottom: 15px; color: #155724;">You can now sign in with your new password.</p>
                            </div>
                            
                            <div style="text-align: center; margin: 30px 0;">
                                <a href="${process.env.NODE_ENV === 'production' ? 'https' : 'http'}://${req.get('host')}" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px 30px; text-decoration: none; border-radius: 25px; font-weight: bold; display: inline-block;">
                                    Sign In to VoicecastingPro
                                </a>
                            </div>
...
// Password reset page endpoint (serves the reset form)
app.get('/reset-password', (req, res) => {
    ...
    // Serve a simple password reset form
    res.send(`
        <!DOCTYPE html>
        <html lang="en">
        <head>
            ...
            <title>Reset Password - VoicecastingPro</title>
        ...
        <body>
            <div class="reset-container">
                <div class="logo">VoicecastingPro</div>
                <h2 style="text-align: center; margin-bottom: 2rem; color: var(--text-primary);">Reset Your Password</h2>
                ...
                <div class="back-link">
                    <a href="/">← Back to VoicecastingPro</a>
                </div>
            </div>
            ...
            <script>
                ...
                form.addEventListener('submit', async function(e) {
                    ...
                    try {
                        ...
                        if (response.ok) {
                            showNotification('Password reset successful! Redirecting to sign in...', 'success');
                            setTimeout(() => {
...
async function startServer() {
    try {
        ...
        // Initialize services
        const servicesInitialized = await initServices();
        
        if (!servicesInitialized) {
            console.log('⚠️ Some services failed to initialize, but server will continue...');
        }
        ...
        app.listen(PORT, '0.0.0.0', () => {
            ...
            console.log(`🗄️ Database: ${pool ? 'Connected to PostgreSQL' : 'Not connected'}`);
        ...
        app.listen(PORT, '0.0.0.0', () => {
            console.log(`🚀 VoicecastingPro server running on port ${PORT} (with limited functionality)`);
        });
    }
}
