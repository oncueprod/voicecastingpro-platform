...
const TalentProfile: React.FC<TalentProfileProps> = ({ talentId, onClose }) => {
  ...
  const checkBackendAuth = async () => {
    ...
    try {
      console.log('🔍 Verifying backend authentication...');
      ...
      console.log('🌐 Backend verification failed:', error);
      return false;
    }
  };
  ...
  const refreshAuthToken = async () => {
    ...
    try {
      ...
      console.log('⚠️ Token refresh failed, trying alternative method...');
  ...
  // COMPLETELY FIXED: Real backend message sending with proper authentication
  const handleSendMessage = async () => {
    ...
    try {
      ...
      let authToken = getAuthToken();
      let isBackendAuth = await checkBackendAuth();
      
      if (!isBackendAuth) {
        console.log('🔄 Refreshing authentication...');
        authToken = await refreshAuthToken();
        isBackendAuth = authToken ? true : false;
      }

      // Step 2: Prepare comprehensive message data
      const messageData = {
      ...
      // Step 3: Try multiple backend endpoints for maximum compatibility
      ...
      for (const endpoint of backendEndpoints) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            setBackendStatus('online'); // Update status on successful send
      ...
      // Step 4: If backend completely failed, try direct email service
      if (!messageSuccess) {
        console.log('🔄 All backend endpoints failed, trying direct email service...');
        
        try {
          const emailData = {
            to: talent.id, // This might be email or will be resolved by backend
            talentName: talent.name,
            from: messageUserId,
            clientName: messageUserName,
            subject: `New Project Inquiry: ${contactForm.subject}`,
            message: `
You have received a new project inquiry from ${messageUserName}.
...
${contactForm.budget ? `Budget: ${contactForm.budget}` : ''}
${contactForm.deadline ? `Deadline: ${contactForm.deadline}` : ''}

Please log into VoiceCastingPro to respond to this inquiry.
            `.trim(),
            type: 'talent_inquiry'
          };
          ...
          if (emailResponse.ok) {
            ...
            setContactForm({ subject: '', message: '', budget: '', deadline: '' });
            setShowContactModal(false);
            setBackendStatus('online'); // Email service is working
            showToast('success', `Message sent successfully to ${talent.name}! They will receive an email notification.`);
            messageSuccess = true;
          }
        } catch (emailError) {
          console.log('❌ Email service also failed:', emailError);
        }
      }

      // Step 5: If everything failed, store locally and show clear message
      if (!messageSuccess) {
        ...
        showToast('error', `Message saved locally but could not reach ${talent.name}. Please try again later or contact support.`);
        
        console.log('⚠️ Message stored locally only - backend services unavailable');
      }
  ...
  return (
    <div className="min-h-screen bg-slate-900 py-8">
      ...
      {(backendStatus === 'offline' || pendingMessageCount > 0) && (
