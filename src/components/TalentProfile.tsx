...
const TalentProfile: React.FC<TalentProfileProps> = ({ talentId, onClose }) => {
  ...
  const checkIfSaved = () => {
    if (!talentId || !canPerformClientActions()) return;
  ...
  const handleContactTalent = () => {
    ...
    if (!isAuthenticated) {
      alert('Please sign in as a client to contact talent.');
      return;
    }
    if (!isClient) {
      alert('Only clients can contact talent. Sign in with a client account to continue.');
      return;
    }
    if (!user?.id) {
  ...
  const handleSendMessage = () => {
    if (!canPerformClientActions()) {
      alert('You must be signed in as a client to send messages.');
  ...
  const handleSaveProfile = () => {
    ...
    if (!isAuthenticated) {
      alert('Please sign in as a client to save talent profiles.');
      return;
    }
    
    if (!isClient) {
      alert('Only clients can save talent profiles. Sign in with a client account to continue.');
      return;
    }
    ...
    if (!isSaved) {
      FavoritesManager.addToGeneralFavorites(user.id, talent.id, talent);
      setIsSaved(true);
      loadTalentStats(); // Refresh stats
      alert(`${talent.name} has been added to your favorites! They'll be notified of your interest.`);
    } else {
      FavoritesManager.removeFromGeneralFavorites(user.id, talent.id);
      setIsSaved(false);
      loadTalentStats(); // Refresh stats
  ...
  const handleAddToShortlist = (projectId: string) => {
    if (!talent || !canPerformClientActions()) {
      alert('You must be signed in as a client to add talent to project shortlists.');
  ...
  const handleAddToProjectClick = () => {
    if (!isAuthenticated) {
      alert('Please sign in as a client to add talent to projects.');
      return;
    }
    if (!isClient) {
      alert('Only clients can add talent to projects. Sign in with a client account to continue.');
      return;
    }
    if (!user?.id) {
      alert('Authentication error. Please try signing in again.');
