...
const TalentProfile: React.FC<TalentProfileProps> = ({ talentId, onClose }) => {
  ...
  // FIXED: Updated handleSendMessage with better error handling and authentication
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (authToken) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            showToast('success', `Message sent successfully to ${talent.name}!`);
            return;
          } else {
            const errorText = await response.text();
            console.warn(`⚠️ Backend API error ${response.status}:`, errorText);
            
            if (response.status === 403) {
              console.log('🔄 Token invalid, clearing and falling back to localStorage...');
      ...
      console.log('📦 Using localStorage and local messaging fallback...');
      
      try {
        // Create message with localStorage only (most reliable method)
        ...
        let conversation = conversations.find(conv => 
          conv.participants && 
          conv.participants.includes(messageUserId) && 
          conv.participants.includes(talent.id)
        );
        
        if (!conversation) {
          conversation = {
            id: `conv_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
            participants: [messageUserId, talent.id],
            participantNames: [messageUserName, talent.name],
            lastMessage: contactForm.subject,
            lastMessageTime: timestamp,
            unreadCount: 0,
            projectTitle: contactForm.subject,
            messages: []
          };
        ...
        if (!saveSuccess) {
          throw new Error('Failed to save message to localStorage');
        }
        ...
        // Optional: Try WebSocket if available (but don't fail if it doesn't work)
        try {
          if (typeof messagingService !== 'undefined' && messagingService.isConnected && messagingService.isConnected()) {
          ...
          console.log('📡 WebSocket send failed (not critical):', wsError);
        }
        ...
        console.log('✅ Message saved successfully using localStorage fallback');
        
      } catch (fallbackError) {
        console.error('❌ localStorage fallback failed:', fallbackError);
        showToast('error', 'Unable to send message. Please check your connection and try again.');
      }
    } finally {
