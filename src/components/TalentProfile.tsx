...
class FavoritesManager {
  ...
  static addToGeneralFavorites(clientId: string, talentId: string, talentData: any) {
    ...
    if (!favorites[clientId].find((fav: any) => fav.talentId === talentId)) {
      ...
      const clientData = JSON.parse(localStorage.getItem('users') || '[]');
      const client = clientData.find((u: any) => u.id === clientId);
...
const TalentProfile: React.FC<TalentProfileProps> = ({ talentId, onClose }) => {
  ...
  const handleSaveProfile = () => {
    ...
    if (!isAuthenticated) {
      console.log('Please sign in as a client to save talent profiles');
      return;
    }
    
    if (!isClient) {
      console.log('Only clients can save talent profiles');
      return;
    }
