...
const TalentProfile: React.FC<TalentProfileProps> = ({ talentId, onClose }) => {
  ...
  // Helper function to get current user ID (prioritize localStorage)
  const getCurrentUserId = () => {
    return localStorage.getItem('userId') || user?.id;
  };

  // Helper function to get current user name (prioritize localStorage)
  ...
  useEffect(() => {
    ...
    (window as any).debugAuth = () => {
      ...
      return { user, isAuthenticated, isClient, isTalent, hasToken: !!authToken, currentUserId, currentUserName };
    };
  }, [talentId, user?.id]);
  ...
  const checkIfSaved = () => {
    const currentUserId = getCurrentUserId();
    if (!talentId || !currentUserId || !canPerformClientActions()) {
      console.log('Cannot check saved status - missing talentId or not authenticated client');
      return;
    }
    console.log('Checking if talent is saved for user:', currentUserId, 'talent:', talentId);
  ...
  const handleSendMessage = async () => {
    ...
    // 🔧 FIX: Use localStorage userId to match Message Center expectations
    const messageUserId = localStorage.getItem('userId') || user.id;
    const messageUserName = localStorage.getItem('userName') || user.name || 'Anonymous Client';
    
    console.log('🆔 USER ID SOURCES:');
    console.log('localStorage.userId:', localStorage.getItem('userId'));
    console.log('auth user.id:', user.id);
    console.log('Using for message:', messageUserId);
    
    console.log('Sending message from:', messageUserId, 'to:', talent.id);
    
    const authToken = getAuthToken();
    console.log('Using auth token:', authToken ? `${authToken.substring(0, 20)}...` : 'No token found');
    
    const messageData = {
      fromId: messageUserId,  // 🔧 Use localStorage userId
      fromName: messageUserName,  // 🔧 Use localStorage userName
      toId: talent.id,
      toName: talent.name,
      ...
      try {
        ...
        if (!conversation) {
          conversation = {
            ...
            participant1: messageUserId,  // 🔧 Use localStorage userId
            participant1Name: messageUserName,  // 🔧 Use localStorage userName
        ...
        if (FavoritesManager.safeSetItem('messages', JSON.stringify(messages)) &&
          ...
          FavoritesManager.createTalentNotification(talent.id, 'message', {
            clientId: messageUserId,  // 🔧 Use localStorage userId
            clientName: messageUserName  // 🔧 Use localStorage userName
          }, undefined, {
            messageId: messageId,
            messageSubject: contactForm.subject
          });
          
          console.log('✅ Message saved with CORRECT USER ID:', newMessage);
          console.log('🆔 Used userId:', messageUserId, 'vs auth userId:', user.id);
          ...
          throw new Error('Failed to save message due to storage quota');
        }
  ...
  const handleSaveProfile = async () => {
    ...
    console.log('Current state:', { isSaved, userId: currentUserId, talentId: talent.id, userName: currentUserName });
