...
const UserProfile: React.FC<UserProfileProps> = ({ onBack }) => {
  ...
  const handleSave = () => {
    try {
      ...
      // Create profile data without image first
      ...
      if (isClient) {
        // Try saving without image first
        try {
          localStorage.setItem('client_profile', JSON.stringify(profileDataWithoutImage));
          console.log('Client profile saved successfully (without image)');
          
          // If profile image exists and is small enough, try to add it
          if (profileImage) {
            ...
            // Test if we can save with image
            localStorage.setItem('client_profile', JSON.stringify(profileDataWithImage));
            console.log('Client profile saved successfully (with image)');
          }
          
        } catch (imageError) {
          console.warn('Could not save with image, saving without:', imageError);
          // Fallback: save without image
          localStorage.setItem('client_profile', JSON.stringify(profileDataWithoutImage));
          alert('Profile saved successfully! Note: Profile image was too large and was not saved. Please use a smaller image.');
        }
      ...
      // Update avatar in auth context (only if save was successful)
      if (profileImage) {
      ...
      if (error.name === 'QuotaExceededError') {
        alert('Storage full! Please:\n\n1. Clear browser data (Settings > Privacy > Clear browsing data)\n2. Or use a smaller profile image\n3. Or try saving without an image for now\n\nYour text data will still be saved.');
        
        // Try to save just the text data without image
        try {
          const textOnlyData = { ...formData };
          delete textOnlyData.profilePhoto;
          
          if (isClient) {
            localStorage.setItem('client_profile', JSON.stringify(textOnlyData));
            alert('Text data saved successfully! Please add a smaller image later.');
            setIsEditing(false);
          }
        } catch (fallbackError) {
          alert('Unable to save even text data. Please clear browser storage and try again.');
        }
      } else {
